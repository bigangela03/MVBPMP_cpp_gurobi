...
//this version print out all to terminal instead of FILE *output
...
//please select ONLY ONE of the following options for calculating fitness
...
GRBEnv *env; // set gurobi environment as a global var
			 ...
			 // if call env.start() in objfunc(), system will make huge number of messages
...
void objfunc(struct individual *critter)
{
	...
	if (UseGurobi)
	{
		...
		int i, j, k, q;
		int status;
		// open Gurobi environment and surpress all output
		// GRBEnv *env = NULL;
		GRBVar **x = NULL;
		GRBVar **theta = NULL;
		x = new GRBVar *[loc];
		theta = new GRBVar *[loc];
		for (i = 0; i < loc; i++)
		{
			x[i] = new GRBVar[loc];
			theta[i] = new GRBVar[loc];
		}
		try
		{
			// env = new GRBEnv ();
			...
			for (i = 0; i < loc; i++)
				for (j = 0; j < loc; j++)
				{
					x[i][j] = model.addVar(
						...
						"x_" + itos(route[i] + 1) + "_" + itos(route[j] + 1));
					theta[i][j] = model.addVar(
						0.0, GRB_INFINITY, 0, GRB_CONTINUOUS,
						"theta_" + itos(route[i] + 1) + "_" + itos(route[j] + 1));
				}
			for (i = 0; i < loc; i++)
			{
				x[i][i].set(GRB_DoubleAttr_UB, 0);
				x[i][0].set(GRB_DoubleAttr_UB, 0);
				x[loc - 1][i].set(GRB_DoubleAttr_UB, 0);
				theta[i][i].set(GRB_DoubleAttr_UB, 0);
				theta[i][0].set(GRB_DoubleAttr_UB, 0);
				theta[loc - 1][i].set(GRB_DoubleAttr_UB, 0);
			}
			// set up constraints
			for (k = 0; k < loc - 1; k++)
			{
				...
				for (i = 0; i <= k; i++)
					for (j = k + 1; j < loc; j++)
						expr += demand[route[i]][route[j]] * x[i][j];
				expr -= theta[k][k + 1];
				model.addConstr(
					expr == 0,
					"flow_" + itos(route[k] + 1) + "_" + itos(route[k + 1] + 1));

				GRBLinExpr expr1 = 0.0;
				expr1 += theta[k][k + 1];
				model.addConstr(
					expr1 <= Q,
					"flow_bound_" + itos(route[k] + 1) + "_" + itos(route[k + 1] + 1));
			}
			...
			for (i = 0; i < loc - 1; i++)
			{
				obj -= C * dis[route[i]][route[i + 1]] * theta[i][i + 1];
				for (j = i + 1; j < loc; j++)
				{
					obj += P * dis[route[i]][route[j]] * demand[route[i]][route[j]] * x[i][j];
				}
			}
			obj -= wv * C * dist;
			...
			{
				double **solx = NULL;
				double **soltheta = NULL;
				solx = new double *[loc];
				soltheta = new double *[loc];
				for (i = 0; i < loc; i++)
				{
					// solx[i] = new double[loc];
					// soltheta[i] = new double[loc];
					for (j = 0; j < loc; j++)
					{
						solx[i] = model.get(GRB_DoubleAttr_X, x[i], loc);
						soltheta[i] = model.get(GRB_DoubleAttr_X, theta[i], loc);
					}
				}
				/*
				 ...
				 if (solx[i][j] > 0.9)
				 printf ("%d -- %d\n", route[i] + 1, route[j] + 1);
				 printf ("===> arc flow (> 0.000001): \n");
				 for (i = 0; i < loc; i++)
				 for (j = 0; j < loc; j++)
				 if (soltheta[i][j] > 0.000001)
				 printf ("%d -- %d (%lf)\n", route[i] + 1, route[j] + 1,
				 soltheta[i][j]);
				 */
				/*for(i=0;i<ncities+1;i++)
				...
				for (i = 0; i < loc - 1; i++)
					for (j = i + 1; j < loc; j++)
					{
						if (solx[i][j] > 0.99)
						{
							critter->cus[num][0] = route[i];
							critter->cus[num][1] = route[j];
							num++;
						}
					}
				critter->cusnum = num;
