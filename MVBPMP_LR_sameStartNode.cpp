...
int main(int argc, char *argv[])
{
	...
	bool startNodesSame = true;
	for (int i = 1; i < numV; i++)
	{
		if (origin[i] != origin[i - 1])
			startNodesSame = false;
	}
	...
	while ((UB - LB) / UB > LR_gap_tolerance)
	{
		...
		// please keep {} below, some vars are defined again later.
		{
			...
			try
			{
...

				{
					...
					// cout << "==========compare var value from veh1 and veh2==============" << endl;
					// for (i = 0; i < n; i++)
					// 	for (j = 0; j < n; j++)
					// 	{
					// 		if (solx_d[i][j][0] > 0 && solx_d[i][j][1] > 0 && (solx_d[i][j][0] != solx_d[i][j][1]))
					// 			printf("%3d %3d  (x1=%lf, x2=%lf)\n", i + 1, j + 1, solx_d[i][j][0], solx_d[i][j][1]);

					// 		if (soly_d[i][j][0] > 0 && soly_d[i][j][1] > 0 && (soly_d[i][j][0] != soly_d[i][j][1]))
					// 			printf("%3d %3d  (y1=%lf, y2=%lf)\n", i + 1, j + 1, soly_d[i][j][0], soly_d[i][j][1]);

					// 		for (k = 0; k < n; k++)
					// 			if (solu_d[i][j][k][0] > 0 && solu_d[i][j][k][1] > 0 && (solu_d[i][j][k][0] != solu_d[i][j][k][1]))
					// 				printf("%3d %3d %3d (u1=%lf, u2=%lf)\n", i + 1, j + 1, k + 1, solu_d[i][j][k][0], solu_d[i][j][k][1]);
					// 	}
					// for (i = 0; i < n; i++)
					// 	if (sols_d[i][0] > 0 && sols_d[i][1] > 0 && (sols_d[i][0] != sols_d[i][1]))
					// 		printf("s%d  (s1=%lf, s2=%lf)\n", i + 1, sols_d[i][0], sols_d[i][1]);
		...
		{ // we assume than if the next best profit is within range below, it is the same as the best one
			if (profit_d[i] >= profit_d[0] - 0.0000000001 && profit_d[i] <= profit_d[0] + 0.0000000001)
				numOptimalProfitSol++;
		}
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
			{
				...
				sum = 0;
				for (int q = 0; q < numOptimalProfitSol; q++)
					sum += soly_d[i][j][q];
				for (int q = numOptimalProfitSol; q < numV; q++)
					sum += soly_d[i][j][0];
				if (sum > 1.9)
				{
					findConflictPickup = true;
