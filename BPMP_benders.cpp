...
public:
  ...
  GRBVar **yv;
  int n;
  printIntSol (GRBVar **xvars, GRBVar **yvars, int nvar, GRBEnv *env)
  {
    xv = xvars;
    yv = yvars;
    n = nvar;
...
protected:
  ...
  callback ()
  {
	...
	if (where == GRB_CB_MIPSOL)
	...
	else if (where == GRB_CB_MIPNODE)
	  {
		...
		//call sub-problem
		GRBVar **pi = new double*[n];
		pi = new GRBVar*[n];
		for (i = 0; i < n; i++)
		  pi[i] = new GRBVar[n];
		subProblem (x, y, n, pi, env);
  ...
  subProblem(double **x, double **y, int n, GRBVar **pi, GRBEnv *env)
    {
      GRBModel SP = GRBModel(*env); //sub-problem model
      ...
      for (k=0; k<n; k++)
	{
	  expr += pi[i][k]+pi[k][j]-pi[i][j];
	  SP.addConstr (expr >= -cost_vehicle*(dis[i][k]+dis[k][j]-dis[i][j]), "dual"+ itos(i)+ "_"+itos (j)+ "_"+itos (k));
	}
      ...
      for (j = 0; j < n; j++)
	{
	  obj += (Q * x[i][j] - wt[i][j]* y[i][j])*pi[i][j];
	}
      ...
      //write model to file
      model.write ("BPMP_benders_SP.lp");
      ...
      if (model.get (GRB_IntAttr_SolCount) > 0)
	{
	  ...
	  for (i = 0; i < n; i++)
	    {
	      sol[i] = model.get (GRB_DoubleAttr_X, pi[i], n);
	      for (int j = 0; j < n; j++)
	      if (sol[i][j] > 0)
	      printf ("pi[%d][%d]\n", i + 1, j + 1, pi[i][j]);
	    }
	...
	if (entry.is_regular_file ())
	  {
		...
		for (i = 0; i < n - 1; i++)
		  for (j = 1; j < n; j++)
		    {
		      obj += (price - cost_flow) * dis[i][j] * wt[i][j]
			  * y[i][j];
		...
		printIntSol cb = printIntSol (x, y, n, env);
