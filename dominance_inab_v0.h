#ifndef DOMINANCE_INAB_V0_H
#define DOMINANCE_INAB_V0_H
    ...
    while (E_toBeTreatedNodesSet.size() > 0) //===> Line 7 in ESPPRC(p)
    {
        ...
        for (auto &node : E_toBeTreatedNodesSetCopy) //===> LINE 9 in ESPPRC(p)
        {
            ...
            for (int succ = 0; succ < n; succ++) //===> LINE 10 in ESPPRC(p)
            {
                ...
                {
                    // vector<double> &label = bigLambda_allNodesLableSet[node][nodeLabelSize - 1 - i];

                    if (label[succ] == 0) //===> LINE 13 in ESPPRC(p) PSEUDO-CODE
                    {
                        if (PRINT_FOR_DEBUG)
                        {
                            cout << "number of labels added in the last iteration = " << labelsAddedInLastIteration[node].size() << endl;
                        ...
                        {
                            // HERE IS A BUG (FINAL SOLUTION IS CORRECT, BUT BIGM DOESN'T WORK ACTUALLY)
                            // BIGM IS ONLY ADDED IN labelsAddedInLastIteration, NOT IN bigLambda_allNodesLableSet
                            // SO BIGM IS NOT AFFECTING NEXT ITERATION.
